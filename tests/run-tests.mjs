...
try {
    ...
    const createTyppisConfig = createConfigModule.default ?? createConfigModule.createTyppisConfig ?? createConfigModule;
    ...
    const typescriptEntry = generated.find((entry) => entry && entry.name === 'typpis/typescript');
    const parserOptions = typescriptEntry?.languageOptions?.parserOptions;
    if (!parserOptions) {
        die('TypeScript entry is missing languageOptions.parserOptions after createTyppisConfig.');
    }
    ...
    if (!Array.isArray(parserOptions.project) || parserOptions.project[0] !== 'tsconfig.json') {
        die('createTyppisConfig did not propagate tsconfigPath.');
    }

    const baseEntry = generated.find((entry) => entry && entry.name === 'typpis/base');
    const resolverProject = baseEntry?.settings?.['import/resolver']?.typescript?.project;
    if (!Array.isArray(resolverProject) || resolverProject[0] !== 'tsconfig.json') {
    ...
    console.log('Config factory entries:', inspect(generated.map((entry) => entry && entry.name).filter(Boolean), { compact: true }));
