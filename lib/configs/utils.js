...
const mergeLanguageOptions = (base = {}, addition = {}) => {
  ...
  merged.parserOptions = deepMerge(
    base.parserOptions || {},
    addition.parserOptions || {},
  );
  merged.parserOptions.project =
    addition.parserOptions?.project || merged.parserOptions.project;
  merged.parserOptions.tsconfigRootDir =
    addition.parserOptions?.tsconfigRootDir ||
    merged.parserOptions.tsconfigRootDir;
...
const extractConfigEntries = (config, parserMap, results = []) => {
  ...
  if (
    ...
    if (config.rules && Object.keys(config.rules).length > 0) {
      const hasSeverityEntries = Object.values(config.rules).some(
        isRuleSeverity,
      );
...
const normalizeConfigEntry = (entry, parserMap) => {
  ...
  normalized.languageOptions = mergeLanguageOptions(
