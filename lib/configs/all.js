...
module.exports = (async () => {
    ...
    for (const [pluginName, rawPlugin] of modules.__pluginEntries) {
        ...
        const strictConfig = gatherStrictPluginConfig(pluginName, resolved, parserMap);
        if (!strictConfig) {
            continue;
        }

        const targetAggregate = TYPE_ONLY_STRICT_PLUGINS.has(pluginName) ? strictTypeAggregate : strictAggregate;

        Object.assign(targetAggregate.rules, prefixPluginRules(pluginName, resolved, strictConfig.rules));
        targetAggregate.settings = deepMerge(targetAggregate.settings, strictConfig.settings);
    ...
    const mergeConfigObject = (configObject) => {
        ...
        strictAggregate.settings = deepMerge(strictAggregate.settings, merged.settings);
    ...
    const manualStrictRules = {
        ...
        'react-hooks/exhaustive-deps': ['error', { additionalHooks: 'use(Callback|Memo|Effect|LayoutEffect)' }]
    };
    ...
    const allConfig = recommended.map((entry) => {
        ...
        if (entry.name === 'typpis/base') {
            return {
                ...
                languageOptions: mergeLanguageOptions(entry.languageOptions || {}, strictAggregate.languageOptions)
            };
        }
    ...
    const prettierIndex = allConfig.findIndex((config) => config === modules.eslintConfigPrettier);
    if (prettierIndex !== -1) {
