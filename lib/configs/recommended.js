...
module.exports = (async () => {
  ...
  const mergeExternalConfig = (configObject) => {
    ...
    baseAggregate.languageOptions = mergeLanguageOptions(
  ...
  for (const [pluginName, rawPlugin] of modules.__pluginEntries) {
    ...
    if (
      !resolvedPlugin ||
      !resolvedPlugin.rules ||
      !Object.keys(resolvedPlugin.rules).length
    ) {
      manualPlugins.add(pluginName);
      continue;
    }

    const pluginConfig = gatherPluginConfig(
      pluginName,
      resolvedPlugin,
      parserMap,
    );
    ...
    Object.assign(
      baseAggregate.rules,
      prefixPluginRules(pluginName, resolvedPlugin, pluginConfig.rules),
    );
    baseAggregate.settings = deepMerge(
      baseAggregate.settings,
      pluginConfig.settings,
    );
    baseAggregate.languageOptions = mergeLanguageOptions(
      baseAggregate.languageOptions,
      pluginConfig.languageOptions,
    );

    if (pluginConfig.plugins) {
      for (const [childName, childPlugin] of Object.entries(
  ...
  if (
    pluginMap.security &&
    modules.pluginSecurity?.rules?.["detect-bidi-characters"]
  ) {
  ...
  if (
    ...
    modules.pluginComments?.recommended?.plugins?.[
  ...
  for (const [pluginName, pluginValue] of typePluginEntries) {
    ...
    Object.assign(
      typeRules,
      prefixPluginRules(pluginName, pluginValue, pluginConfig.rules),
    );
    typeSettings = deepMerge(typeSettings, pluginConfig.settings);
    typeLanguageOptions = mergeLanguageOptions(
      typeLanguageOptions,
      pluginConfig.languageOptions,
    );
  ...
  typeRules["deprecation/deprecation"] =
    typeRules["deprecation/deprecation"] || "warn";
  ...
  const tsconfigPath = modules.path.resolve(
    __dirname,
  ...
  const recommendedConfig = [
    ...
    },
    {
      ...
      languageOptions: mergeLanguageOptions(
        {},
        { parser: modules.eslintMdx?.Parser ?? modules.eslintMdx },
      ),
    ...
    },
    {
      ...
      rules:
        modules.html.configs["flat/recommended"]?.rules ||
        modules.html.configs.recommended.rules,
    },
    {
      name: "typpis/json",
      files: ["**/*.json"],
      languageOptions: mergeLanguageOptions(
        {},
        { parser: modules.jsoncEslintParser },
      ),
      plugins: {
        json: modules.json,
      },
    ...
    },
    {
      ...
      languageOptions: mergeLanguageOptions(
        {},
        { parser: modules.yamlEslintParser },
      ),
      plugins: {
    ...
    },
    {
      ...
      languageOptions: mergeLanguageOptions(
        {},
        { parser: modules.tomlEslintParser },
      ),
      plugins: {
  ...
  [
    ...
    "testing-library",
    "playwright",
  ].forEach((handled) => manualPlugins.delete(handled));
