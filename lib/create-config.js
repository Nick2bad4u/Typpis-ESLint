...
const applyTypeScriptProjects = (configEntries, projects, rootDir) => {
  const typescriptEntry = configEntries.find(
    (entry) => entry?.name === "typpis/typescript",
  );
...
const applyImportResolverProjects = (configEntries, projects, rootDir) => {
  const baseEntry = configEntries.find(
    (entry) => entry?.name === "typpis/base",
  );
  if (!baseEntry || !baseEntry.settings) {
    return;
  }

  const resolverSettings = /** @type {Record<string, unknown> | undefined} */ (
...
const createTyppisConfig = async (options = {}) => {
  ...
  if (projects.length === 0) {
    throw new Error(
      "Typpis config requires at least one tsconfigPath entry when using typed rules.",
    );
  }

  const config = /** @type {TyppisFlatConfigArray} */ (
    base.map(cloneConfigEntry)
  );
